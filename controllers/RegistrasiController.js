...
export const RegisterUser = async (req, res) => {
  ...
  try {
    ...
    if (existingUser) {
      return res
        .status(400)
        .json({ message: "User dengan NIM tersebut sudah terdaftar." });
    }
    const existtingEmail = await Akun.findOne({ where: { email } });
    if (existtingEmail) {
      return res
        .status(400)
        .json({ message: "User dengan Email tersebut sudah terdaftar." });
    }
    ...
    if (tanggalSekarang < tanggalLahir) {
      return res.status(400).json({
        code: 400,
        status: "error",
        message: "Tanggal Lahir Tidak Benar",
      });
    }
...
`,
    ...
    return res
      .status(500)
      .json({ code: 500, message: error.errors[0].message });
  }
};
  ...
  try {
    ...
    if (akunmahasiswa.nim !== nim) {
      if (existingUserWithNim && existingUserWithNim.nim === nim) {
        return res
          .status(400)
          .json({ message: "NIM Sudah Digunakan Oleh User Lain" });
      }
    }
    const tanggalLahir = new Date(tanggal_lahir);
    const tanggalSekarang = new Date();
...
export const RegisterSuperAdmin = async (req, res) => {
  ...
  try {
    const ceksuperadmin = await Akun.findOne({
      where: { AksesRole: "Super Admin" },
    });
    let status = false;
