...
export const CreateSoalUjianList = async (req, res) => {
  ...
  try {
    ...
    const existingSoal = await SoalUjian.findAll({
      where: { idSoal: idSoal, idUjian: idUjian },
    });
    ...
    const exitsSoal = await SoalUjian.findAll({ where: { idUjian: idUjian } });
    if (exitsSoal.length === 0) {
      const tipeSoal = await BankSoal.findAll({
        where: {
          id: idSoal,
        },
        attributes: ["id", "tipe_soal"],
      });
      const uniqueTypes = [...new Set(tipeSoal.map((soal) => soal.tipe_soal))];
      if (uniqueTypes.length === 1) {
        return res.status(400).json({
          code: 400,
          status: "error",
          message: "Inputkan Tipe Soal Multiple dan Essay",
        });
      }
    }
...
export const GetSoalUjianId = async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({
      code: 500,
      status: "error",
      message: "Terjadi kesalahan saat proses Mengambil Soal",
    });
  }
};
...
export const DeleteDataSoalUjianList = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ code: 200, status: "success", message: "Data Berhasil Dihapus" });
  } catch (error) {
    console.error("Error saat proses Menghapus Soal: ", error);
    return res.status(500).json({
      code: 500,
      status: "error",
      message: "Terjadi kesalahan saat proses Menghapus Soal",
    });
  }
};
